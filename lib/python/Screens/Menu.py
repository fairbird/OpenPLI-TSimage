...
def MenuEntryPixmap(entryID, png_cache, lastMenuID):
    ...
    if png is None:
        png = LoadPixmap(resolveFilename(SCOPE_CURRENT_SKIN, 'menu/' + entryID + '.png'))
        if png is None:
...
class MenuSort(Menu):
    ...
    def __init__(self, session, parent):
        ...
        self['MoveActions'] = ActionMap(['WizardActions', 'DirectionActions'], {'moveUp': boundFunction(self.moveChoosen, -1),
         'moveDown': boundFunction(self.moveChoosen, +1)}, -1)
        self['EditActions'] = ActionMap(['ColorActions'], {'red': self.closeMenuSort,
         'green': self.keySave,
