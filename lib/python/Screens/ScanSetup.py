...
from enigma import eTimer, eDVBFrontendParametersSatellite, eComponentScan, eDVBSatelliteEquipmentControl, eDVBFrontendParametersTerrestrial, eDVBFrontendParametersCable, eConsoleAppContainer, eDVBResourceManager, eDVBFrontendParametersATSC
...
cable_bands = {'DVBC_BAND_EU_VHF_I': 1 << 0,
...
cable_autoscan_nimtype = {'SSH108': 'ssh108',
 'TT3L10': 'tt3l10',
 'TURBO': 'vuplus_turbo_c'}
terrestrial_autoscan_nimtype = {'SSH108': 'ssh108_t2_scan',
...
class CableTransponderSearchSupport():
    ...
    def getCableTransponderData(self, str):
        ...
        for line in lines:
            ...
            if len(data):
                ...
                tmpstr += '\n\n'
                tmpstr += data[1].isdigit() and '%s MHz ' % (int(data[1]) / 1000.0) or data[1]
                tmpstr += data[0]
    ...
    def startCableTransponderSearch(self, nim_idx):
        ...
        if tunername == 'CXD1981':
            ...
            cmd = '%(BIN_NAME)s --init --scan --verbose --wakeup --inv 2 --bus %(BUS)d' % {'BIN_NAME': bin_name,
             'BUS': bus}
...
class TerrestrialTransponderSearchSupport():
    ...
    def terrestrialTransponderInitSearchList(self, searchList, region):
        ...
        for x in tpList:
            if x[0] == 2:
                freq = x[1]
                bandWidth = self.terrestrialTransponderConvBandwidth_I(x[2])
                parm = (freq, bandWidth)
                searchList.append(parm)

    def terrestrialTransponderConvBandwidth_I(self, _bandWidth):
        bandWidth = {eDVBFrontendParametersTerrestrial.Bandwidth_8MHz: 8000000,
         ...
         eDVBFrontendParametersTerrestrial.Bandwidth_10MHz: 10000000}.get(_bandWidth, 8000000)
        return bandWidth

    def terrestrialTransponderconvBandwidth_P(self, _bandWidth):
        bandWidth = {8000000: eDVBFrontendParametersTerrestrial.Bandwidth_8MHz,
         ...
         10000000: eDVBFrontendParametersTerrestrial.Bandwidth_10MHz}.get(_bandWidth, eDVBFrontendParametersTerrestrial.Bandwidth_8MHz)
        return bandWidth
    ...
    def terrestrialTransponderSearch(self, freq, bandWidth):
        ...
        cmd = '%s --freq %d --bw %d --bus %d --ds 2' % (self.terrestrial_search_binName,
         freq,
         bandWidth,
         self.terrestrial_search_bus)
...
class ScanSetup(ConfigListScreen, Screen, CableTransponderSearchSupport, TerrestrialTransponderSearchSupport):
    ...
    def __init__(self, session):
        ...
        self['actions'] = NumberActionMap(['SetupActions', 'MenuActions', 'ColorActions'], {'ok': self.keyGo,
         'save': self.keyGo,
    ...
    def newConfig(self):
        ...
        print 'cur is', cur
        if cur == self.typeOfScanEntry or cur == self.typeOfInputEntry or cur == self.tunerEntry or cur == self.systemEntry or cur == self.preDefSatList or cur == self.TerrestrialRegionEntry or cur == self.TerrestrialCompleteEntry or self.modulationEntry and self.systemEntry[1].value == eDVBFrontendParametersSatellite.System_DVB_S2 and cur == self.modulationEntry:
            self.createSetup()

    def createConfig(self, frontendData):
        defaultSat = {'orbpos': 192,
        ...
        self.scan_sat.system = ConfigSelection(default=defaultSat['system'], choices=[(eDVBFrontendParametersSatellite.System_DVB_S, _('DVB-S')), (eDVBFrontendParametersSatellite.System_DVB_S2, _('DVB-S2'))])
        self.scan_sat.frequency = ConfigInteger(default=defaultSat['frequency'], limits=(1, 99999))
        self.scan_sat.inversion = ConfigSelection(default=defaultSat['inversion'], choices=[(eDVBFrontendParametersSatellite.Inversion_Off, _('Off')), (eDVBFrontendParametersSatellite.Inversion_On, _('On')), (eDVBFrontendParametersSatellite.Inversion_Unknown, _('Auto'))])
        self.scan_sat.symbolrate = ConfigInteger(default=defaultSat['symbolrate'], limits=(1, 99999))
        self.scan_sat.polarization = ConfigSelection(default=defaultSat['polarization'], choices=[(eDVBFrontendParametersSatellite.Polarisation_Horizontal, _('horizontal')),
         (eDVBFrontendParametersSatellite.Polarisation_Vertical, _('vertical')),
         (eDVBFrontendParametersSatellite.Polarisation_CircularLeft, _('circular left')),
         (eDVBFrontendParametersSatellite.Polarisation_CircularRight, _('circular right'))])
        self.scan_sat.fec = ConfigSelection(default=defaultSat['fec'], choices=[(eDVBFrontendParametersSatellite.FEC_Auto, _('Auto')),
        ...
        self.scan_sat.fec_s2 = ConfigSelection(default=defaultSat['fec_s2'], choices=[(eDVBFrontendParametersSatellite.FEC_Auto, _('Auto')),
         (eDVBFrontendParametersSatellite.FEC_1_2, '1/2'),
         (eDVBFrontendParametersSatellite.FEC_2_3, '2/3'),
         (eDVBFrontendParametersSatellite.FEC_3_4, '3/4'),
        ...
        self.scan_sat.modulation = ConfigSelection(default=defaultSat['modulation'], choices=[(eDVBFrontendParametersSatellite.Modulation_QPSK, 'QPSK'),
        ...
        self.scan_sat.rolloff = ConfigSelection(default=defaultSat.get('rolloff', eDVBFrontendParametersSatellite.RollOff_alpha_0_35), choices=[(eDVBFrontendParametersSatellite.RollOff_alpha_0_35, '0.35'),
         (eDVBFrontendParametersSatellite.RollOff_alpha_0_25, '0.25'),
         (eDVBFrontendParametersSatellite.RollOff_alpha_0_20, '0.20'),
         (eDVBFrontendParametersSatellite.RollOff_auto, _('Auto'))])
        self.scan_sat.pilot = ConfigSelection(default=defaultSat.get('pilot', eDVBFrontendParametersSatellite.Pilot_Unknown), choices=[(eDVBFrontendParametersSatellite.Pilot_Off, _('Off')), (eDVBFrontendParametersSatellite.Pilot_On, _('On')), (eDVBFrontendParametersSatellite.Pilot_Unknown, _('Auto'))])
        self.scan_sat.is_id = ConfigInteger(default=defaultSat.get('is_id', 0), limits=(0, 255))
        self.scan_sat.pls_mode = ConfigSelection(default=defaultSat['pls_mode'], choices=[(eDVBFrontendParametersSatellite.PLS_Root, _('Root')), (eDVBFrontendParametersSatellite.PLS_Gold, _('Gold')), (eDVBFrontendParametersSatellite.PLS_Combo, _('Combo'))])
        self.scan_sat.pls_code = ConfigInteger(default=defaultSat.get('pls_code', 1), limits=(0, 262142))
        self.scan_cab.frequency = ConfigInteger(default=defaultCab['frequency'], limits=(50, 999))
        self.scan_cab.inversion = ConfigSelection(default=defaultCab['inversion'], choices=[(eDVBFrontendParametersCable.Inversion_Off, _('Off')), (eDVBFrontendParametersCable.Inversion_On, _('On')), (eDVBFrontendParametersCable.Inversion_Unknown, _('Auto'))])
        self.scan_cab.modulation = ConfigSelection(default=defaultCab['modulation'], choices=[(eDVBFrontendParametersCable.Modulation_QAM16, '16-QAM'),
        ...
        self.scan_cab.fec = ConfigSelection(default=defaultCab['fec'], choices=[(eDVBFrontendParametersCable.FEC_Auto, _('Auto')),
        ...
        self.scan_cab.system = ConfigSelection(default=defaultCab['system'], choices=[(eDVBFrontendParametersCable.System_DVB_C_ANNEX_A, _('DVB-C')), (eDVBFrontendParametersCable.System_DVB_C_ANNEX_C, _('DVB-C ANNEX C'))])
        ...
        self.scan_ter.inversion = ConfigSelection(default=defaultTer['inversion'], choices=[(eDVBFrontendParametersTerrestrial.Inversion_Off, _('Off')), (eDVBFrontendParametersTerrestrial.Inversion_On, _('On')), (eDVBFrontendParametersTerrestrial.Inversion_Unknown, _('Auto'))])
        self.scan_ter.bandwidth = ConfigSelection(default=defaultTer['bandwidth'], choices=[(1712000, '1.712MHz'),
         (5000000, '5MHz'),
         (6000000, '6MHz'),
         (7000000, '7MHz'),
         (8000000, '8MHz'),
         (10000000, '10MHz')])
        self.scan_ter.fechigh = ConfigSelection(default=defaultTer['fechigh'], choices=[(eDVBFrontendParametersTerrestrial.FEC_1_2, '1/2'),
        ...
        self.scan_ter.feclow = ConfigSelection(default=defaultTer['feclow'], choices=[(eDVBFrontendParametersTerrestrial.FEC_1_2, '1/2'),
         (eDVBFrontendParametersTerrestrial.FEC_2_3, '2/3'),
         (eDVBFrontendParametersTerrestrial.FEC_3_4, '3/4'),
         (eDVBFrontendParametersTerrestrial.FEC_5_6, '5/6'),
         (eDVBFrontendParametersTerrestrial.FEC_6_7, '6/7'),
         ...
         (eDVBFrontendParametersTerrestrial.FEC_8_9, '8/9'),
         (eDVBFrontendParametersTerrestrial.FEC_Auto, _('Auto'))])
        self.scan_ter.modulation = ConfigSelection(default=defaultTer['modulation'], choices=[(eDVBFrontendParametersTerrestrial.Modulation_QPSK, 'QPSK'),
        ...
        self.scan_ter.transmission = ConfigSelection(default=defaultTer['transmission_mode'], choices=[(eDVBFrontendParametersTerrestrial.TransmissionMode_1k, '1K'),
        ...
        self.scan_ter.guard = ConfigSelection(default=defaultTer['guard_interval'], choices=[(eDVBFrontendParametersTerrestrial.GuardInterval_1_32, '1/32'),
        ...
        self.scan_ter.hierarchy = ConfigSelection(default=defaultTer['hierarchy'], choices=[(eDVBFrontendParametersTerrestrial.Hierarchy_None, _('None')),
        ...
        self.scan_ter.system = ConfigSelection(default=defaultTer['system'], choices=[(eDVBFrontendParametersTerrestrial.System_DVB_T_T2, _('Auto')), (eDVBFrontendParametersTerrestrial.System_DVB_T, _('DVB-T')), (eDVBFrontendParametersTerrestrial.System_DVB_T2, _('DVB-T2'))])
        ...
        self.scan_ats.inversion = ConfigSelection(default=defaultATSC['inversion'], choices=[(eDVBFrontendParametersATSC.Inversion_Off, _('Off')), (eDVBFrontendParametersATSC.Inversion_On, _('On')), (eDVBFrontendParametersATSC.Inversion_Unknown, _('Auto'))])
        self.scan_ats.modulation = ConfigSelection(default=defaultATSC['modulation'], choices=[(eDVBFrontendParametersATSC.Modulation_Auto, _('Auto')),
        ...
        self.scan_ats.system = ConfigSelection(default=defaultATSC['system'], choices=[(eDVBFrontendParametersATSC.System_ATSC, _('ATSC')), (eDVBFrontendParametersATSC.System_DVB_C_ANNEX_B, _('DVB-C ANNEX B'))])
        self.scan_scansat = {}
        ...
        self.scan_type = ConfigSelection(default=defaultSatSearchType, choices=[('single_transponder', _('User defined transponder')),
        ...
        self.cable_toggle = {'single_transponder': 'complete',
         'complete': 'single_transponder'}
    ...
    def addATSCTransponder(self, tlist, frequency, modulation, inversion, system):
        print 'Add ATSC frequency: %s inversion: %s modulation: %s system: %s' % (frequency,
         modulation,
         inversion,
         system)
        parm = eDVBFrontendParametersATSC()
        ...
        if nim.isCompatible('DVB-S'):
            ...
            if self.scan_type.value == 'single_transponder':
                ...
                if len(nimsats):
                    ...
                    self.addSatTransponder(tlist, self.scan_sat.frequency.value, self.scan_sat.symbolrate.value, self.scan_sat.polarization.value, fec, self.scan_sat.inversion.value, orbpos, self.scan_sat.system.value, self.scan_sat.modulation.value, self.scan_sat.rolloff.value, self.scan_sat.pilot.value, self.scan_sat.is_id.value, self.scan_sat.pls_mode.value, self.scan_sat.pls_code.value)
                removeAll = False
            ...
            if self.scan_typecable.value == 'single_transponder':
                self.addCabTransponder(tlist, self.scan_cab.frequency.value * 1000, self.scan_cab.symbolrate.value * 1000, self.scan_cab.modulation.value, self.scan_cab.fec.value, self.scan_cab.inversion.value)
                removeAll = False
            elif self.scan_typecable.value == 'predefined_transponder':
            ...
            if self.scan_type_atsc.value == 'single_transponder':
                self.addATSCTransponder(tlist, frequency=self.scan_ats.frequency.value * 1000, modulation=self.scan_ats.modulation.value, inversion=self.scan_ats.inversion.value, system=self.scan_ats.system.value)
                removeAll = False
            elif self.scan_type_atsc.value == 'predefined_transponder':
    ...
    def predefinedTranspondersList(self, orbpos):
        ...
        if orbpos is not None:
            ...
            if self.scan_sat.system.value == eDVBFrontendParametersSatellite.System_DVB_S2:
                ...
                fec = self.scan_sat.fec.value
            compare = [0,
             self.scan_sat.frequency.value * 1000,
             self.scan_sat.symbolrate.value * 1000,
    ...
    def startScan(self, tlist, flags, feid, networkid = 0):
        if len(tlist):
            if self.finished_cb:
                self.session.openWithCallback(self.finished_cb, ServiceScan, [{'transponders': tlist,
                  'feid': feid,
                  'flags': flags,
                  'networkid': networkid}])
            else:
                self.session.openWithCallback(self.startScanCallback, ServiceScan, [{'transponders': tlist,
                  'feid': feid,
                  'flags': flags,
                  'networkid': networkid}])
        elif self.finished_cb:
...
class ScanSimple(ConfigListScreen, Screen, CableTransponderSearchSupport, TerrestrialTransponderSearchSupport):
    ...
    def __init__(self, session):
        ...
        self['key_red'] = StaticText(_('Close'))
        self['key_green'] = StaticText(_('Scan'))
        self['actions'] = ActionMap(['SetupActions', 'MenuActions', 'ColorActions'], {'ok': self.keyGo,
         'save': self.keyGo,
         'cancel': self.keyCancel,
        ...
        if n:
            if n.value:
                ...
                if action == APPEND_NOW:
                    self.scanList.append({'transponders': tlist,
                     'feid': nim.slot,
                     'flags': flags})
                else:
                    if action == SEARCH_CABLE_TRANSPONDERS:
                        self.flags = flags
                        self.feid = nim.slot
                        self.networkid = networkid
                        self.startCableTransponderSearch(nim.slot)
                        return
                    if action == SEARCH_TERRESTRIAL2_TRANSPONDERS:
                        self.tlist = tlist
                        self.flags = flags
                        self.feid = nim.slot
    ...
    def setTerrestrialTransponderSearchResult(self, tlist):
        ...
        if self.tlist is not None:
            self.scanList.append({'transponders': self.tlist,
             'feid': self.feid,
             'flags': self.flags})
